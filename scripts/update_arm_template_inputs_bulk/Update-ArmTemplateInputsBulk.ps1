...
.DESCRIPTION
    ...
    yet on ARM version 4.7 and redeploys them with the updated ARM template and function
    package. Supports dry-run (what-if) mode, parallel batch deployments, and per-input
    error handling with optional auto-skip. ARM 4.7 configures each Function App to use
    a system-assigned managed identity for Event Hub access.
...
.PARAMETER ForceInputIds
    ...
    tagged 4.7 but other resources failed). These inputs are normally excluded from discovery.
...
.NOTES
    ...
    Authorization:
    - The operator needs permission to create subscription deployments, update the deployed
      resources, and create Azure role assignments. ARM 4.7 grants the Function App identity
      the Azure Event Hubs Data Receiver role.
...
# This allows reprocessing inputs that were partially upgraded (e.g. function app tagged 4.7
