...
Bulk upgrades Splunk Data Manager Azure Monitor Logs inputs to ARM version 4.7.
...
The core operation is: **read the existing input configuration, then re-apply the 4.7 ARM template with the same required deployment parameters**. HEC credentials, service principal, and resource group tags are read from the live resources and passed through unchanged. The 4.7 template updates the Function App to use a system-assigned managed identity for Event Hub access.

Resource Logs inputs are not discovered or upgraded by this script. The 4.7 Resource Logs template has no parameter or deployment-structure changes from the previous release.
...
- Permission to create subscription deployments, update the deployed resources, and create Azure role assignments
...
> **Authorization requirement for 4.7:** The AAD and Activity Logs templates grant each Function App's managed identity the `Azure Event Hubs Data Receiver` role. The operator therefore needs `Microsoft.Authorization/roleAssignments/write` at the Event Hub namespace or a parent scope.
...
| Resource group tags | `Get-AzResourceGroup` — merged with `SplunkInputARMVersion=4.7` |
...
Template URIs:
- AAD Logs: `release/4.7/deploy/aad/splunk-aad-logs-deploy-resources.json`
- Activity Logs: `release/4.7/deploy/activity/splunk-activity-logs-deploy-resources.json`

Function package:
- `https://download.splunk.com/products/splunk-azure-monitor-logs-azure-function/releases/4.7.0/linux/azure-monitor-logs-azure-function-4.7.0.zip`
...
Deployments use **Incremental mode** — existing resources are updated in place and resources omitted from the template are not deleted. The Event Hub, consumer group, and jobs storage account keep their existing names and resource identities, so the Event Hub trigger continues to use the existing checkpoint storage.

ARM 4.7 changes Event Hub authentication for AAD and Activity Logs inputs:

- enables a system-assigned managed identity on the Function App
- grants that identity the `Azure Event Hubs Data Receiver` role on the Event Hub namespace
- replaces the SAS-based `EventHubConnection` app setting with `EventHubConnection__fullyQualifiedNamespace`
- deploys Azure Function package `4.7.0`

Azure role assignments can take several minutes to propagate. The Function App might temporarily pause Event Hub processing during that window. Because the existing consumer group and checkpoint storage are preserved, processing resumes from the stored checkpoint after the role becomes effective. Run `-DryRun` first and monitor Function App and Event Hub metrics during the actual upgrade.

Re-running the script is safe — inputs already upgraded to 4.7 are excluded from discovery by the Resource Graph query (`SplunkInputARMVersion != '4.7'`).

### Partial upgrade failure risk

ARM deploys resources sequentially based on `dependsOn` ordering. If the deployment fails after the Function App is successfully updated (and tagged `4.7`) but before a later resource (for example, the Event Hub role assignment or diagnostic settings) is updated, the input will be **excluded from subsequent discovery runs** because the function app tag already shows `4.7`.
...
Monitor in-flight deployments per subscription with the Azure-provided tools.
